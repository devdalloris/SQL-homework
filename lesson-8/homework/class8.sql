...
SELECT MIN(StepNumber) as 'Min Step Number', MAX(StepNumber) as 'Max Step Number', [Status], COUNT(*) AS ConsecutiveCount
FROM (
    SELECT *,
...
select HireYear, 
	...
	CASE 
		...
		when IntervalYear>2 AND NextYear is not null then concat(HireYear+1, '-', NextYear-1)
		when NextYear is null then concat(HireYear+1, '-', '2025')
	end as Years
from (select 
			year(HIRE_DATE) as HireYear,
			lead(year(HIRE_DATE)) over(order by HIRE_DATE) as NextYear,
			lead(year(HIRE_DATE)) over(order by HIRE_DATE)-year(HIRE_DATE) as IntervalYear from EMPLOYEES_N) as t
where 
	 (CASE 
		when IntervalYear <2 or HireYear >1975 then null
		when IntervalYear>2 AND NextYear is not null then concat(HireYear+1, '-', NextYear-1)
